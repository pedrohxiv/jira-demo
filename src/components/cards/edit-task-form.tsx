...
export const EditTaskForm = ({ onCancel, taskId }: Props) => {
  const params = useParams<{ workspaceId: string }>();

  const { data: task } = getTask({ taskId });
  ...
  const handleFormSubmit = (values: z.infer<typeof updateTaskSchema>) => {
    mutate(
      { param: { taskId }, json: { ...values } },
      {
        onSuccess: () => {
          onCancel?.();
        },
